 ...
 */
void SerialPort::SetTransParam_InputModes( struct termios *pTransParam, Type4Parity Parity)
{
 ...
 * Opis flag patrz: man tcsetattr
 */
void SerialPort::SetTransParam_OutputModes(struct termios *pTransParam )
{
 ...
 */
void SerialPort::SetTransParam_ControlModes( struct termios *pTransParam, Type4Parity Parity, Type4CharacterSize CSize)
{
...
void SerialPort::SetTransParam_LocalModes( struct termios *pTransParam )
{
 ...
 */
bool SerialPort::SetTransParam(int PortDesc, Type4BaudRate BRate, Type4Parity Parity, Type4CharacterSize CSize)
{
...
bool SerialPort::RS232_Send(int PortDescriptor, const uint8_t *Data)
{
    write(PortDescriptor, Data, 4);
    return true;
}

void SerialPort::ConfiguratePort(const char *sDevName)
{
    if ((PortDescriptor = open(sDevName, O_RDWR | O_NONBLOCK)) < 0)
    {
        std::cerr << "Blad otwarcia portu !!!" << sDevName << std::endl;
        exit(1);
    }
    SetTransParam(PortDescriptor);
}

void SerialPort::Send(uint8_t *Val)
{
  RS232_Send(PortDescriptor, Val);
}
